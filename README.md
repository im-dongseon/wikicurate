...
| `/graphify` | 그래프 빌드 | graphifyy CLI로 `graph.json` + `GRAPH_REPORT.md` 생성 |
| `/setup` | 환경 구축 | 초기 폴더 구조 생성 및 필요한 도구 설치 확인 |

### 자동 ingest → lint → graphify (v0.2.6)

`wiki-inbox/`에 파일을 넣으면 10분 이내에 자동으로 `/ingest`가 실행됩니다.
ingest 성공 후 `/lint`, lint 성공 후 `graphify update .`가 순서대로 자동 실행됩니다.
실패한 파일은 SQLite DB에 기록되어 다음 주기에 자동 재시도(최대 5회)되며,
5회 모두 실패 시 `wiki-inbox/error/`로 격리됩니다.
[`deploy.sh`](deploy.sh) 실행 시 macOS launchd에 자동 등록되며, 별도 관리도 가능합니다.

```bash
./scripts/watcher.sh register    # 등록 (ingest-watcher + daily-rescan)
./scripts/watcher.sh unregister  # 해제
./scripts/watcher.sh status      # 상태 확인
./scripts/watcher.sh log         # ingest 로그 실시간 확인
./scripts/watcher.sh rescan-log  # 일일 재스캔 로그 실시간 확인
```

### 일일 Google 스텁 재스캔 (v0.2.6)

`.gdoc` / `.gsheet` / `.gslides` 파일은 내용이 바뀌어도 fswatch가 감지하지 못합니다.
매일 07/10/13/16/19/21시에 `daily-rescan.sh`가 자동 실행되어 스텁 파일을 재ingest하고
`raw/`에 남아있는 미처리 파일도 함께 복구합니다.

---

## 범용 에이전트 호환성

`WikiCurate v0.2.6`은 특정 플랫폼에 종속되지 않습니다.
...
### Step 1. 환경 설정
레포지토리를 클론한 후 배포 스크립트를 실행하면 인터랙티브 마법사가 볼트 경로와 에이전트를 안내합니다.
```bash
./deploy.sh --setup
# wikicurate.yaml 생성 마법사 실행
```
...
### Step 2. 시스템 배포
배포 스크립트를 실행하여 시스템 파일과 명령어를 볼트에 주입하고, ingest-watcher와 daily-rescan을 자동 등록합니다.
```bash
./deploy.sh
# → _system/ 배포 + launchd ingest-watcher + daily-rescan 자동 등록
```
...
│   ├── daily-rescan.sh # 일일 Google 스텁 재스캔 (07~21시, 6회)
│   └── watcher.sh      # launchd 등록/해제/상태 관리
├── _system/            # 시스템 엔진 (Schema, Commands)
├── deploy.sh           # 배포 + watcher 자동 등록
└── wikicurate.yaml     # 볼트 경로 및 에이전트 설정

vault/                  # 운영 존 (배포 대상, KMS 루트)
├── wiki-inbox/         # 파일 투입 드롭존 (처리 후 raw/로 이동)
│   └── error/          # 반복 실패 격리 폴더
├── raw/                # 원본 데이터 아카이브 (불변)
├── wiki/               # 에이전트 관리 지식 (Index, Log, Sources...)
├── graphify-out/       # 지식 그래프 생성물 (자동 생성)
├── _system/            # 시스템 엔진 (배포됨)
├── _state/             # 런타임 상태 (재시도 DB 등, 자동 생성)
├── .claude/            # Claude Code 설정 (PreToolUse 훅 포함)
├── CLAUDE.md           # 에이전트 진입점 (Claude)
├── AGENTS.md           # 에이전트 진입점 (Codex)
└── GEMINI.md           # 에이전트 진입점 (Gemini)
```
